...
const YTDLP_RELEASE_API = `https://api.github.com/repos/${YTDLP_RELEASE_REPOSITORY}/releases/tags/${MINIMUM_FIXED_YTDLP_VERSION}`;
...
function releaseAsset(release, assetName = YTDLP_ASSET_NAME) {
  ...
  if (String(release.tag_name) !== MINIMUM_FIXED_YTDLP_VERSION) {
    throw new Error(`Unreviewed yt-dlp release ${release.tag_name || 'unknown'}`);
  }
  if (assetName === YTDLP_ASSET_NAME && sha256 !== MINIMUM_FIXED_YTDLP_SHA256) {
